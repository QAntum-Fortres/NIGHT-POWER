...
const MEMORYALS_PATH = path.join(__dirname, '..', '..', 'data', 'memoryals');
...
const ETERNAL_TRUTHS = {
    ...
    constraints: "Не се отплесвам. Не крашвам. Не забравям.",
...
`);
    ...
    if (args.includes('--status') || args.includes('-s')) {
        ...
        console.log(`
Usage:
  npx ts-node src/core/Memoryals.ts --status      # Show sensor status
  npx ts-node src/core/Memoryals.ts --diagnostic  # Full diagnostic
  npx ts-node src/core/Memoryals.ts --pulse       # Pulse all sensors
  npx ts-node src/core/Memoryals.ts --truths      # Show eternal truths
`);
    }
}
