...
export class PineconeContextBridge extends EventEmitter {
  ...
  /**
   * Upsert vectors directly to Pinecone (for Neural Absorption)
   */
  async upsert(records: Array<{
    ...
    if (!records || records.length === 0) {
      this.log('warn', '⚠️ No records to upsert');
      return;
    }

    try {
      this.log('info', `📤 Upserting ${records.length} records to namespace: ${this.config.namespace}...`);

      await this.index.namespace(this.config.namespace).upsert(records);

      this.log('success', `✅ Upsert complete: ${records.length} records processed.`);
    } catch (error) {
